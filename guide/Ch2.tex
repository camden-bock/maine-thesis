...
The first command tells the class file where the bibliography entries are located.  This should be a \textsc{Bib}\TeX\ file (i.e. one with a ``.bib'' extension).  If you're unfamiliar with \textsc{Bib}\TeX\ then you'll need to familiarize your self with it \parencite{Feder:2006}, or one of the various programs designed to help you manage a \textsc{Bib}\TeX\ file (e.g. Bibdesk \parencite{BibDesk} for Mac OS X, Referencer \parencite{Spray:2007} for Linux, and BibDB \parencite{Doron:1999} for Windows).
